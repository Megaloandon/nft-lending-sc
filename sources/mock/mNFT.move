module lending_addr::lending {
    ...
    public entry fun update_creator(sender: &signer, new_creator: address) acquires Config {
        ...
        let config = borrow_global_mut<Config>(@lending_addr);
    ...
    public entry fun create_fa(
        ...
        let sender_addr = signer::address_of(sender);
        let config = borrow_global<Config>(@lending_addr);
        assert!(is_admin(config, sender_addr) || is_creator(config, sender_addr), EONLY_ADMIN_OR_CREATOR_CAN_CREATE_FA);

        let fa_owner_obj_constructor_ref = &object::create_object(@lending_addr);
        ...
        let registry = borrow_global_mut<Registry>(@lending_addr);
    ...
    public fun get_registry(): vector<Object<Metadata>> acquires Registry {
        let registry = borrow_global<Registry>(@lending_addr);
        registry.fa_objects
    }
    ...
    fun is_admin(config: &Config, sender: address): bool {
        if (sender == config.admin_addr) {
            ...
            if (object::is_object(@lending_addr)) {
                let obj = object::address_to_object<ObjectCore>(@lending_addr);
    ...
    #[test(aptos_framework = @0x1, sender = @lending_addr)]
    fun test_happy_path(
